...
if (!isset($_FILES['pdf-escaneado']) || $_FILES['pdf-escaneado']['error'] === UPLOAD_ERR_NO_FILE) {
    ...
    header("Location: ../for-cuatro.php?status=no_changes");
    exit();
}

if ($_FILES['pdf-escaneado']['error'] !== UPLOAD_ERR_OK) {
    header("Location: ../for-cuatro-edit.php?status=upload_error");
    exit();
}
...
if ($tipoArchivo !== 'application/pdf') {
    header("Location: ../for-cuatro-edit.php?status=invalid_format");
    exit();
}
...
$sql_get_id = "SELECT id FROM documento_cuatro WHERE usuario_id = ? ORDER BY id DESC LIMIT 1";
...
if ($row = $result->fetch_assoc()) {
    ...
    $sql_update = "UPDATE documento_cuatro SET pdf_escaneado = ? WHERE id = ? AND usuario_id = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("sii", $rutaParaDB, $documento_id, $usuario_id);

    if ($stmt_update->execute()) {
        header("Location: ../for-cuatro.php?status=update");
    } else {
        header("Location: ../for-cuatro-edit.php?status=db_error");
    }
    $stmt_update->close();
} else {
    header("Location: ../for-cuatro-edit.php?status=not_found");
    exit();
}
